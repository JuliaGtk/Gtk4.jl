...
function _precompile_()
    ...
    precompile(Tuple{typeof(setindex!),GtkWindow,GtkWidget})
    precompile(Tuple{typeof(cursor),GtkWidget,String})
    precompile(Tuple{Type{GtkGestureClick}})
    precompile(Tuple{typeof(getgc),GtkCanvas})
    precompile(Tuple{typeof(height),GtkWidget})
    precompile(Tuple{typeof(width),GtkWidget})
end
